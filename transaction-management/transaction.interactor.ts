...
export class TransactionInteractor implements IInteractor {
  menu = new Menu("\nTransaction-Management", [
    ...
    { key: "6", label: "Total no of transaction" },
      ...
      if (op) {
        switch (op?.key.toLocaleLowerCase()) {
          ...
          case "5":
            ...
            break;
          case "6":
            const count = await this.repo.getTotalCount();
            console.log(count);
          case "7":
...
async function issueBook(
  ...
  const createdTransaction: ITransaction | null =
...
async function returnBook(repo: TransactionRepository) {
  while (true) {
    ...
    const transaction: ITransaction | null = await repo.getById(transactionId);
    if (transaction) {
      ...
      if (status) {
        if (transaction.Status === "Issued") {
          await repo.update(transaction.id, transaction);
          console.log(
            chalk.green(
...
async function todaysDueList(repo: TransactionRepository) {
  ...
  const formattedTodaysDate = formatDate(today).split(",")[1].trim();
  const transactions: ITransaction[] = await repo.getAllTransactions();
  const dueToday = transactions.filter((transaction) => {
    const formattedDate = transaction.dueDate.split(",")[1].trim();
